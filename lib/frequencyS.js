      ...
      if (i == 0) { // first item
        ...
        dataArr.forEach((val, ind) => {
          if ((val > start || val.indexOf(start) == 0) && (val < end || val.indexOf(end) == 0)) {
            (frequencyRanges[`${start}-${end}`] == undefined) ?
              (frequencyRanges[`${start}-${end}`] = 1) :
              (frequencyRanges[`${start}-${end}`] += 1)
            delete dataArr[ind]
          }
        })
      } else if (i == (intervalArr.length - 1)) { // last item
        let lastItem = alphabet.indexOf(intervalArr[i - 1]) + 1
        let start = alphabet[lastItem]
        let end = item
        ...
        let end = item
        dataArr.forEach((val, ind) => {
          if ((val > start || val.indexOf(start) == 0) &&
            (val < end || val.indexOf(end) == 0)) {
            (frequencyRanges[`${start}-${end}`] == undefined) ?
              (frequencyRanges[`${start}-${end}`] = 1) :
              (frequencyRanges[`${start}-${end}`] += 1)
            delete dataArr[ind]
